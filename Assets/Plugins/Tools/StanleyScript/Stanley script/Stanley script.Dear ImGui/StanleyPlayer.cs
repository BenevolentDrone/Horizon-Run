...
namespace HereticalSolutions.StanleyScript
{
	public class StanleyPlayer
	{
		...
		private readonly IInterpreterContext executable;
		...
		public StanleyPlayer(
			...
			IInterpreterContext executable,
			ScenarioNameScriptPair[] scenarioPlaylist)
		{
			...
			this.reportMaker = executable as IReportMaker;
		...
		private void DrawPlaybackControls()
		{
			...
			ImGui.Text($"Status: {executable.ExecutionStatus}");
		...
		private void DrawScenarioListing()
		{
			...
			if (!string.IsNullOrEmpty(currentScenarioName))
			{
				...
				int currentLine = executable.LineCounter - 1; //-1 because ANTLR4's AST starts with line 1, not line 0
		...
		private void DrawReportListing()
		{
			...
			var report = reportMaker.GetReport();
