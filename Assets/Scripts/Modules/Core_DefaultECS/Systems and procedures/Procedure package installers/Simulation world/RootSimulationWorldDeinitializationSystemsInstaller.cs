...
{
	public class RootSimulationWorldDeinitializationSystemsInstaller
		: IProcedurePackageInstaller<TSystem, TProcedure>
	{
		...
		public string PackageName => SystemConstsDefaultECS.SIMULATION_WORLD_DEINITIALIZATION_PACKAGE_INSTALLER_ID;
		...
		{
			if (context.Builders.Has(SystemConstsDefaultECS.SIMULATION_WORLD_DEINITIALIZATION_SYSTEM_BUILDER_ID))
				return false;
			...
			IProcedurePackageInstallerContext<TSystem, TProcedure> context)
		{
			...
			if (!context.Builders.TryAdd(
				SystemConstsDefaultECS.SIMULATION_WORLD_DEINITIALIZATION_SYSTEM_BUILDER_ID,
				systemBuilder))
			{
				logger?.LogError(
					GetType(),
					$"FAILED TO ADD SYSTEM BUILDER {SystemConstsDefaultECS.SIMULATION_WORLD_DEINITIALIZATION_SYSTEM_BUILDER_ID}");

				return;
			}

			systemBuilder.AddStageNodesBetweenStartAndFinish(
				SystemConstsDefaultECS.SIMULATION_WORLD_DEINITIALIZATION_SYSTEMS_ID);

			//simulationWorldController.EntityDeinitializationSystems = systemBuilder.BuildSystem();
		}
