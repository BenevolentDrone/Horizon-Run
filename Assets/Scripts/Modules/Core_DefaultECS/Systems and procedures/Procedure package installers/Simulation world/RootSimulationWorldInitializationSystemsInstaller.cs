...
{
	public class RootSimulationWorldInitializationSystemsInstaller
		: IProcedurePackageInstaller<TSystem, TProcedure>
	{
		...
		public RootSimulationWorldInitializationSystemsInstaller(
		...
		public string PackageName => SystemConstsDefaultECS.SIMULATION_WORLD_INITIALIZATION_PACKAGE_INSTALLER_ID;
		...
		{
			if (context.Builders.Has(SystemConstsDefaultECS.SIMULATION_WORLD_INITIALIZATION_SYSTEM_BUILDER_ID))
				return false;
			...
			IProcedurePackageInstallerContext<TSystem, TProcedure> context)
		{
			...
			if (!context.Builders.TryAdd(
				SystemConstsDefaultECS.SIMULATION_WORLD_INITIALIZATION_SYSTEM_BUILDER_ID,
				systemBuilder))
			{
				logger?.LogError(
					GetType(),
					$"FAILED TO ADD SYSTEM BUILDER {SystemConstsDefaultECS.SIMULATION_WORLD_INITIALIZATION_SYSTEM_BUILDER_ID}");

				return;
			}

			systemBuilder.AddStageNodesBetweenStartAndFinish(
				SystemConstsDefaultECS.SIMULATION_WORLD_INITIALIZATION_SYSTEMS_ID);

			//simulationWorldController.EntityInitializationSystems = systemBuilder.BuildSystem();
		}
