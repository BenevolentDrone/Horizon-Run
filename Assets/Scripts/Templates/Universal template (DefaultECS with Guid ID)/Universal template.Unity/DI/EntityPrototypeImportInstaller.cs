...
namespace HereticalSolutions.Templates.Universal.Unity.Installers
{
	public class EntityPrototypeImportInstaller : MonoInstaller
	{
		...
		private bool TryGetEntityWorldsRepository(
			out IReadOnlyEntityWorldsRepository<World,IDefaultECSEntityWorldController> entityWorldsRepository,
			ILogger logger = null)
		{
			...
			ILogger logger = null)
		{
			...
			var worldWithPrototype = worldController as IPrototypeCompliantWorldController<World, Entity>;

			if (worldWithPrototype == null)
			{
				logger?.LogError<EntityPrototypeImportInstaller>(
					$"worldController CANNOT BE CAST TO IPrototypeCompliantWorldController<World, Entity>");
